 ...
 * @param {string} path - 儲存路徑
 * @param {Function} onProgress - 上傳進度回調（可選）
 * @returns {Promise<string[]>} 圖片 URL 陣列
 */
export const uploadMultipleImages = async (files, path, onProgress = null) => {
  try {
    ...
    console.log(`📤 開始批次上傳 ${files.length} 張圖片`);
    ...
    const uploadPromises = files.map(async (file, index) => {
      try {
        ...
        console.error(`❌ 圖片 ${index + 1} 上傳失敗:`, error);
        return null;
      }
    });
    ...
    console.log(`✅ 批次上傳完成：${successUrls.length}/${files.length} 成功`);
...
export const deleteMultipleImages = async (imageUrls) => {
  try {
    ...
    console.log(`🗑️ 開始批次刪除 ${imageUrls.length} 張圖片`);
    ...
    console.log(`✅ 批次刪除完成：成功 ${success}，失敗 ${failed}`);
...
export const uploadStoreImage = async (file, storeId, type) => {
  try {
    const compressedFile = await compressImage(file, 800, 600, 0.85);
 ...
 * @returns {Promise<string>} 圖片 URL
 */
export const uploadItemMainImage = async (file, storeId, type, itemId) => {
  try {
 ...
 * @returns {Promise<string[]>} 圖片 URL 陣列
 */
export const uploadItemGalleryImages = async (files, storeId, type, itemId, onProgress = null) => {
  try {
    // 壓縮所有圖片
    console.log('📦 開始壓縮相簿圖片...');
    const compressedFiles = await Promise.all(
      files.map(file => compressImage(file, 600, 600, 0.8))
    );

    // 批次上傳
    const path = `stores/${type}/${storeId}/items/${itemId}/gallery`;
    const urls = await uploadMultipleImages(compressedFiles, path, onProgress);
    
    return urls;
  } catch (error) {
    console.error('❌ 批次上傳商品相簿失敗:', error);
    throw error;
  }
};