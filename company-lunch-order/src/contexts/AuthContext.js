  ...
  const loginWithGoogle = async () => {
    try {
      ...
      if (isMobile) {
        ...
        devLog('📱 [DEBUG] User-Agent:', navigator.userAgent);
        devLog('📱 [DEBUG] 視窗寬度:', window.innerWidth);
        devLog('📱 [DEBUG] 當前 URL:', window.location.href);

        await signOut(auth).catch(() => { });
        devLog('📱 [DEBUG] 已清除舊登入狀態，準備跳轉到 Google...');
    ...
    const handleRedirectResult = async () => {
      ...
      try {
        ...
        if (result?.user) {
          ...
          localStorage.removeItem('pendingAuth');
          localStorage.removeItem('authAttemptTime');
          devLog('🧹 已清除 pendingAuth 標記（getRedirectResult）');
    ...
    const setupAuthListener = () => {
      ...
      unsubscribe = onAuthStateChanged(auth, async (user) => {
        ...
        devLog('👤 onAuthStateChanged 觸發:', {
          ...
          authAttemptTime: authAttemptTime ? new Date(parseInt(authAttemptTime)).toLocaleTimeString() : null
        });
        ...
        if (user) {
          ...
          // 🔥 關鍵修正：立即清除 pendingAuth 標記
          if (pendingAuth === 'true') {
            localStorage.removeItem('pendingAuth');
            localStorage.removeItem('authAttemptTime');
            devLog('🧹 已清除 pendingAuth 標記（onAuthStateChanged）');
          }
          ...
          // 🔥 關鍵修正：檢查是否還在等待登入
          if (pendingAuth === 'true' && authAttemptTime) {
            const elapsed = Date.now() - parseInt(authAttemptTime);
            const maxWaitTime = 20000; // 20 秒超時（原本 10 秒）

            if (elapsed < maxWaitTime) {
              devLog(`⏳ 還在等待登入完成（已等待 ${Math.round(elapsed / 1000)} 秒 / ${maxWaitTime / 1000} 秒）`);
              setLoginStatusMessage(`正在完成登入... (${Math.round(elapsed / 1000)}s)`);
              // 🔥 不要設定為 null，繼續等待
              return;
            } else {
              devWarn('⚠️ 登入等待超時（超過 10 秒），清除標記');
              localStorage.removeItem('pendingAuth');
              localStorage.removeItem('authAttemptTime');
              setLoginStatusMessage('');
            }
