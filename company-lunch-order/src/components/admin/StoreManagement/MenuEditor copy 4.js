...
import { Save, X, Plus, FolderInput } from 'lucide-react';
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-7xl max-h-[90vh] flex flex-col">
        ...
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center z-10">
          <div>
            ...
            {hasChanges && (
              <p className="text-sm text-orange-600 mt-1">?? 有未儲存的變更</p>
            )}
          </div>
                      ...
                      <div key={category.id} className={`border rounded-lg overflow-hidden ${
                        ...
                        <div className={`p-4 flex items-center justify-between ${
                          ...
                          <h4 className={`font-bold ${isUncategorized ? 'text-orange-800' : 'text-gray-900'}`}>
                            {isUncategorized ? '??' : '??'} {category.name} ({category.items?.length || 0})
                          </h4>
                          ...
                          {!isUncategorized && (
                            <div className="flex gap-1">
                              ...
                              {isSuperAdmin() && (
                                ...
                                >
                                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a22 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
</svg>
</button>
)}
</div>
