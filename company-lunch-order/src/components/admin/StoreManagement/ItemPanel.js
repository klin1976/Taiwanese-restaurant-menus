...
const ItemPanel = ({
  ...
  // ✅ 新增：處理「未分類商品」按鈕點擊
  const handleUncategorizedPickerClick = () => {
    // 檢查是否為「未分類」分類
    if (isUncategorized) {
      alert('請先選擇一個目標分類（非「未分類」）');
      return;
    }
    ...
    <div className="flex-1 flex flex-col bg-white overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-200 bg-gray-50">
        ...
        <div className="flex flex-wrap gap-2">
          {/* ✅ 修改：未分類商品按鈕（顯示目標分類、加上驗證） */}
          {!isUncategorized && (
            <button
              onClick={handleUncategorizedPickerClick}
              ...
              title={
                uncategorizedCount > 0 
                  ? `從未分類挑選商品到「${category.name}」` 
                  : '沒有未分類商品'
              }
            >
              <Package size={16} />
              <span className="hidden sm:inline">未分類商品</span>
              <span className="sm:hidden">未分類</span>
              {uncategorizedCount > 0 && (
                <>
                  ...
                  {/* ✅ 新增：顯示目標分類 */}
                  <span className="hidden md:inline text-xs text-orange-600">
                    → {category.name}
                  </span>
                </>
              )}
        ...
        {items.length === 0 ? (
          <div className="text-center py-12">
            ...
            {!isUncategorized && uncategorizedCount > 0 && (
              <button
                onClick={handleUncategorizedPickerClick}
                className="mt-4 px-4 py-2 bg-orange-100 text-orange-700 rounded-lg hover:bg-orange-200 transition-colors text-sm font-medium inline-flex items-center gap-2"
              >
                <Package size={16} />
                從「未分類」挑選商品 ({uncategorizedCount}) → {category.name}
              </button>
            )}
          </div>
        ) : (
