...
- 辨識常見分類：主食、單點、湯品、小菜等。
- **組合品項解析**：若有「主食+飲料」或「主餐+三樣菜」的組合，請在 description 中簡述組合內容。
- **手寫字體識別**：部分手寫便當菜單若字跡模糊，請根據上下文（字首或字尾）進行最接近的語法推斷。`;
    ...
    async (request) => {
        ...
        try {
            ...
            const model = vertexAI.getGenerativeModel({
                model: 'gemini-1.5-flash', // 使用穩定版名稱
            });


            // 組裝 Prompt
            let prompt = MENU_RECOGNITION_PROMPT;
            if (finalStoreType === "drinks") {
            ...
            console.log("Calling Gemini 1.5 Flash API via Vertex AI SDK...");
            ...
            // 從 Vertex AI Result 中提取文字
            const resultText = response.response.candidates[0].content.parts[0].text;
            let textContent = resultText || "";
